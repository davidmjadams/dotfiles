...
public:
	...
	virtual int SendWhiteboardPdu(uint32_t targetId, const uint8_t* data, uint32_t data_len)
	{
		return 0;
	}
...
public:
	...
	virtual int UpdateCloudWbSetting(
		IN CloudWbSetting wb_setting,
		IN void* p_data,
		IN int32_t size_of_data
		) = 0;
	
	virtual int FlushWbCapturer(	
		) = 0;

	virtual int SetWbViewport(
		IN const void* viewport
		) = 0;

	virtual int GetWbViewport(
		OUT void* viewport
		) = 0;
		
	virtual int ReceiveWhiteboardPdu(
		IN uint32_t sender_id,
		IN const uint8_t* data,	// caller must release the memory
		IN uint32_t data_len
		) = 0;

	virtual int CtrlLinuxTool(
		IN const void* data
	) = 0;
